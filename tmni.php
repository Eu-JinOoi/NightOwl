...
<script>
	...
	{		
		...
		ret+="<div id='xpander-"+pjson.PID+"' class='xpander close' style='position:absolute; bottom:0px; right:0;' onClick='xpand("+pjson.PID+");'>";//can be openDetails to open sliding page
		//ret+="<a class=''>";
	...
	function handleJSON(json,page)
	{
		...
		if(json.places.length!=0)
		{
			...
			$("#scrollableContent").html(fdata);//+json.places.length+" results were returned."+fdata);
		}
		else
		{
			$("#scrollableContent").html("<div style='margin:80px;'><h3 style='color:#000;'>Um....you must live in the middle of nowhere...or we haven't gottent your area yet. We couldn't find anything in this category.</h3></div>");	
		}
	}
		...
		$.getJSON("https://maps.googleapis.com/maps/api/geocode/json?latlng="+position.coords.latitude+","+position.coords.longitude+"&sensor=true",function( data ) {
			...
			for(var i=0; i<data.results.length;i++)
			{
				if(data.results[i].types[0]=="locality" && data.results[i].types[1]=="political")
				{
					...
					{
						scity=(data.results[i].formatted_address).substr(0,(data.results[i].formatted_address).indexOf(",",0));
			...
			if($("#locationDesc").length==0)
			{
				...
				$("#searchingNear").html(scity+"<div style=' margin:10px; padding:10px; width:48px; height:48px; background-image:url(/resources/images/donotuse/location.png); background-position:center center; background-repeat:no-repeat; display:inline;'>&nbsp;</div>");
			}
			else
			{
	...
	{
		alert("Load the Categories");	
	}
</script>
</head>

<body id='bodytag' style="background-color:#e5e5e5;">
	<div id='brandingBar' style="height:8em; background-image:url(resources/images/orange-county.jpg); background-position:bottom center; background-repeat:no-repeat;">
    		<div style="width:48px; height:48px; position:absolute; top:2px; left:0; cursor:pointer;" >
                	<div class='border-menu'>&nbsp;</div>
                </div>
            <div style="margin:auto; font-size:.8em; margin-top:1.3em;">
            Looking for <span id='categories' onClick='openCategories();'><u>everything</u></span> near
            <div id='searchingNear' style="font-size:2.2em; margin-top:.2em;">&nbsp;</div>
            </div>
        	<?php /*<a id='menubutton'>
            	<div style="width:48px; height:48px; position:absolute; top:2px; left:0; cursor:pointer;" >
                	<div class='border-menu'>&nbsp;</div>
	...
	<div id='maincont'>
        ...
        <div class='clear'>&nbsp;</div>
        <div id='pagearea' style="position:relative; margin-top:12.2em;">
